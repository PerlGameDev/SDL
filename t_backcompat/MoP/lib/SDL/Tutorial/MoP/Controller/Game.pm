...
sub notify {
    ...
    my $method = $event_method{$state}{ $event->{name} }
      || $event_method{$state}{'*'};
...
sub _create_new_piece {
    ...
    my ( $x, $y ) =
      SDL::Tutorial::MoP::Model::Pieces->init_xy( $self->{piece},
        $self->{pieceRotation} );
    ...
    #     //  Next piece
    ( $self->{next_piece}, $self->{next_rotation} ) =
      SDL::Tutorial::MoP::Model::Pieces->random();
}
