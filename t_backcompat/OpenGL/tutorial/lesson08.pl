...
print STDERR
  "Use b to toggle blend, page up/down to zoom, and arrow keys to rotate\n";
...
sub InitGL {
    ...
    my $LightAmbient =
      [ 0.5, 0.5, 0.5, 1.0 ];  # white ambient light at half intensity (rgba) */

    my $LightDiffuse =
      [ 1.0, 1.0, 1.0, 1.0 ];   # super bright, full intensity diffuse light. */
...
sub ImageLoad {
    ...
    my $surface =
      SDL::Surface->new( -name => $filename );    #makes use of SDL: BMP loader.
