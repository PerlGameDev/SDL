...
BEGIN {
	...
	plan( skip_all => 'GNU Hurd not supported' )
		if $^O eq 'gnu';
...
SKIP:
{
	...
	ok( defined $states{$status},
		"[SDL::CD->status] is " . ( defined $states{$status} ? $states{$status} : 'undefined' )
	);
	SKIP:
	{
		...
		ok( defined $types{$t_type},
			"[SDL::CDTrack->type] is " . ( defined $types{$t_type} ? $types{$t_type} : 'undefined' )
		);
