...
isa_ok( SDL::GFX::Rotozoom::surface( $src, 0, 1, 0 ),
    'SDL::Surface', 'surface' );
draw();
my ( $dest_w, $dest_h ) =
  @{ SDL::GFX::Rotozoom::surface_size( 100, 200, 45, 1 ) };
...
( $dest_w, $dest_h ) =
  @{ SDL::GFX::Rotozoom::surface_size( 100, 200, 45, 0.3 ) };
...
SKIP:
{
    ...
    isa_ok( SDL::GFX::Rotozoom::surface_xy( $src, 1, 1, 1, 1 ),
        'SDL::Surface', 'surface_xy' );
    draw();
}
( $dest_w, $dest_h ) =
  @{ SDL::GFX::Rotozoom::surface_size_xy( 100, 200, 45, 1.3, 1.7 ) };
is( $dest_w > 100,
...
( $dest_w, $dest_h ) =
  @{ SDL::GFX::Rotozoom::surface_size_xy( 100, 200, 45, 0.3, 0.2 ) };
is( $dest_w < 100,
...
isa_ok( SDL::GFX::Rotozoom::zoom_surface( $src, 1, 1, 1 ),
    'SDL::Surface', 'zoom_surface' );
draw();
( $dest_w, $dest_h ) =
  @{ SDL::GFX::Rotozoom::zoom_surface_size( 100, 200, 0.5, 0.7 ) };
is( $dest_w < 100,
    1, 'zoom_surface_size, resulting width decreases at zoom 0.5' );
is( $dest_h < 200,
    1, 'zoom_surface_size, resulting height decreases at zoom 0.7' );
( $dest_w, $dest_h ) =
  @{ SDL::GFX::Rotozoom::zoom_surface_size( 100, 200, 1.2, 7.7 ) };
...
SKIP:
{
    ...
    isa_ok( SDL::GFX::Rotozoom::shrink_surface( $src, 1, 1 ),
...
SKIP:
{
    ...
    isa_ok( SDL::GFX::Rotozoom::rotate_surface_90_degrees( $src, 1 ),
...
sub draw {
    ...
    SDL::Video::blit_surface(
        ...
        $display,
        SDL::Rect->new(
            50 * 20, 100, $surface->w + 50 + 20, $surface->h + 100
        )
    );
