...
The C<unicode> field is only used when UNICODE translation is enabled with SDL_EnableUNICODE. 
...
The button state can be interpreted using the C<SDL_BUTTON> macro (see SDL_GetMouseState). 
...
when this event occured is stored in the C<button_x> and the C<button_y> fields. Like SDL_KeyboardEvent, 
