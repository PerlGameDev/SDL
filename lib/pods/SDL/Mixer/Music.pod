...
This callback is called when music called by e.g. L<play_music> or L<fade_in_music> stops naturally. This happens when the music is over or is 
fading out.

B<Note>: If you play music via L<hook_music>, this callback will never be called.
...
Returns the C<position> (first) parameter that will be passed to L<hook_music>'s callback.
