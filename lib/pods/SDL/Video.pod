...
The flags parameter is passed to C<SDL::Surface-E<gt>new()> and has those semantics.
This function is used internally by C<SDL::Video::display_format>.
This function can only be called after C<SDL::init>. 
 
it returns a C<SDL::Surface> on success or C<undef> on error.
...
blitting onto the display surface. It calls C<SDL::Video::convert_surface>.
...
suitable for fast blitting onto the display surface. It calls C<SDL::Video::convert_surface>.
...
B<Note>: The video surface must be initialised using C<SDL::Video::set_video_mode> before this function is called, or it will segfault.

=head2	load_BMP(filename)

 $surface = SDL::Video::load_BMP( $filename );

Loads a C<SDL::Surface> from a named Windows BMP file.
C<SDL::Video::load_BMP> returns a C<SDL::Surface> on success or C<undef> on error.
...
Saves the given C<SDL::Surface> as a Windows BMP file named filename. 
...
the key value).
The key must be of the same pixel format as the surface, C<SDL::Video::map_RGB> is often useful for obtaining an acceptable value.
If flag is C<SDL_SRCCOLORKEY> then key is the transparent pixel value in the source image of a blit.

If C<flag> is OR'd with C<SDL_RLEACCEL> then the surface will be drawn using RLE acceleration when drawn with SDL::Blit_surface.
The surface will actually be encoded for RLE acceleration the first time C<SDL::Video::blit_surface> or C<SDL::Video::display_format> 
...
The color should be a pixel of the format used by the surface, and can be generated by the C<SDL::Video::map_RGB> or C<SDL::Video::map_RGBA>
...
If there is a clip rectangle set on the destination (set via C<SDL::Video::set_clip_rect>), then this function will clip based on the 
intersection of the clip rectangle and the dstrect rectangle, and the dstrect rectangle will be modified to represent the area actually filled.

If you call this on the video surface (ie: the value of C<SDL::Video::get_video_surface>) you may have to update the video surface to see the 
...
The results of blitting operations vary greatly depending on whether C<SDL_SRCALPHA> is set or not. See C<SDL::Video::set_alpha> for an 
...
For an example see L<load_BMP|/load_BMP_filename>.
...
If you wish, you may load the OpenGL library from the given path at runtime, this must be done before C<SDL::Video::set_video_mode> is called.
You must then use C<SDL::Video::GL_get_proc_address> to retrieve function pointers to GL functions. 
...
C<SDL::Video::GL_load_library>, then all GL functions must be retrieved this way. Usually this is used to retrieve function pointers to OpenGL 
extensions. Note that this function needs an OpenGL context to function properly, so it should be called after C<SDL::Video::set_video_mode>
has been called (with the C<SDL_OPENGL> flag).
...
It returns SDL/OpenGL attribute C<attr>. This is useful after a call to C<SDL::Video::set_video_mode> to check whether your attributes have 
been set as you expected. 
C<SDL::Video::GL_get_attribute> returns undef if the attribute is not found.
...
You should use C<GL_get_attribute> to check the values after a C<set_video_mode> call, since the values obtained can differ from the 
requested ones.
...
Much the same as C<SDL::Video::lock_surface>, C<lock_YUV_overlay> locks the overlay for direct access to pixel data.
...
The opposite to C<SDL::Video::lock_YUV_overlay>. Unlocks a previously locked overlay. An overlay must be unlocked before it can be displayed. 
C<unlock_YUV_overlay> does not return anything.

=head2	display_YUV_overlay(overlay,dstrect)

 $display_overlay = SDL::Video::display_YUV_overlay( $overlay, $dstrect );

Blit the overlay to the display surface specified when the overlay was created. The C<SDL::Rect> structure, C<dstrect>, specifies a rectangle 
...
This function must be called before the first call to C<SDL::Video::set_video_mode>. Note that this means C<SDL::Image> cannot be used.
