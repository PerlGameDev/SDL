...
C<set_video_mode> returns a L<SDL::Surface> on success or it returns undef on error, the error message is retrieved using C<SDL::get_error>.
...
variables SDL_VIDEO_CENTERED=center or SDL_VIDEO_WINDOW_POS=x,y. You can also set them via C<SDL::putenv>.
