...
B<Important>: All methods except C<new_file> and C<new_const_mem> are still TODO.

=head1 METHODS


=head2 new_file(file,mode) 

C<new_file> creates a new SDL::RWOps structure for reading from and/or writing to a named file. 
...
=head2 new_const_mem
  ...
  my $rw = SDL::RWOps->new_const_mem( $image_data, $size );

C<new_const_mem> sets up a SDL::RWOps object based on a memory area of a certain size. The C<$size> parameter is optional.
