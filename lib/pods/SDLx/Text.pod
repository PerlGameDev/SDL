...
and low-level functions scattered all over the place. This is a sugar
...
For example:
   ...
   my $string = $text->text;        # gets the current text
   $text->text( 'new text' );       # sets a new text
...
C<[R, G, B, A]> format. See L<SDL::Color> for more information on colors.
...
Returns the underlying surface representing the text itself. You probably don't
need this, unless you're doing something really funky.
...
Please report any bugs or feature requests to the bug tracker. We will be
...
This module ships with a default font, "I<Gentium Basic>",
Copyright 2003-2008 SIL International (http://sil.org),
released under the SIL Open Font License 1.1 (OFL).
The font is available for use in free and commercial products, with some minor
restrictions.
Please read the C<OFL.txt> and C<OFL-FAQ.txt> for further information.
