...
The C<SDLx::App> provides methods for the root window of your SDL application, as well as many related convenience methods.
...
C<SDLx::App> is a C<SDLx::Controller>. Use the event, show and handlers to run the app.
 ...
 # run the app with the added callbacks
 $app->run();

For a full explanation of the L<run|SDLx::Controller/run> loop and other Controller related methods, please see L<SDLx::Controller>.
