...
C<min_t> specifies the minimum time, in seconds, that has to accumulate before any move or show handlers are called, and defaults to 0.
Setting this to C<1 / 60> ensures that the controller can update the screen at a maximum of 60 times per second.
...
C<event> is a SDL::Event object that events going to the event callbacks are polled in to. Defaults to C<SDL::Event->new()>.
...
activate the main loop. The main loop will run until C<quit> is called.
...
Note that the last argument each callback recieves is the C<SDLx::Controller> object.
...
If the callback returns a false value, C<pause> will indefinitely wait for more events, repeating the process, until the callback returns true.
...
The second is the C<SDLx::Controller> object.
 ...
 $controller->add_event_handler(\&quit);
...
The argument can be 0 if no time has passed since the last cycle. Set a C<min_t> if you need to protect against this.

The second argument passed to the callbacks is the total amount of C<dt> passed. The third is the C<SDLx::Controller> object.
