...
sub draw_rect {
	...
	Carp::croak "Rect needs to be a SDL::Rect or an array ref"
		unless ref($rect) eq 'ARRAY' || $rect->isa('SDL::Rect');
	require Scalar::Util;
	my $pass_color = $color;
	if ( Scalar::Util::looks_like_number($color) ) {
		...
		$pass_color = ( $color->r << 24 ) + ( $color->g << 16 ) + ( $color->b << 8 ) + 0xFF;
	} else {
	...
	my $result = SDL::Video::fill_rect( $self->surface, $prect, $pass_color );
