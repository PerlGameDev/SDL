...
sub midleft {
    ...
    if ( defined $centery ) {
        $self->top( $centery - ( $self->height >> 1 ) )
          ;    # y = centery - (height/2)
    }
    return;
}
...
sub center {
    ...
    if ( defined $centerx ) {
        $self->left( $centerx - ( $self->width >> 1 ) )
          ;    # x = centerx - (width/2)
    }
    if ( defined $centery ) {
        $self->top( $centery - ( $self->height >> 1 ) )
...
sub midright {
    ...
    return ( $self->top + ( $self->height >> 1 ),
        $self->left + $self->width )                     # (centery, right)
...
sub bottomright {
    ...
    return ( $self->top + $self->height,
        $self->left + $self->width )    # (bottom, right)
...
sub midbottom {
    ...
    return ( $self->left + ( $self->width >> 1 ),
        $self->top + $self->height )    # (centerx, bottom)
